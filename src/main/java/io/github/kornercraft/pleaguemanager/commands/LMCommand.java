package io.github.kornercraft.pleaguemanager.commands;

import co.aikar.commands.BaseCommand;
import co.aikar.commands.CommandHelp;
import co.aikar.commands.annotation.*;
import io.github.kornercraft.pleaguemanager.LeagueManager;
...
public class LMCommand extends BaseCommand {
  ...
  public void onToggle(CommandSender sender) {
    ...
    getInstance().getServer().broadcastMessage(Lang.TOGGLE.getConfigValue(new String[]{state,sender.getName()}));
  }
  ...
  public void onBan(CommandSender sender, String[] args) {
    if (args.length < 1) {
      ...
      if (target != null && target.hasPlayedBefore()) {
        ...
        Node node = Node.builder("leaguemanager.banned").value(true).expiry(time.toMilliseconds(), TimeUnit.MILLISECONDS).build();
  ...
  public void onSuspend(CommandSender sender, String[] args) {
    if (args.length < 1) {
      ...
      if (target != null && target.hasPlayedBefore()) {
        ...
        Node node = Node.builder("group.suspend").value(true).expiry(time.toMilliseconds(), TimeUnit.MILLISECONDS).build();
        DataMutateResult result = user.data().add(node);
